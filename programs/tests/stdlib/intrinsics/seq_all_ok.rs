...
pub fn seq_all(s: Seq<Integer>, t: Seq<Integer>, x: Integer, y: Integer) -> Boolean {
    ...
    let _extract: Seq<Integer> = Seq::extract(
        Seq::concat(Seq::unit(x), Seq::unit(y)),
        Integer::from(0),
        Integer::from(1),
    );
    let _index_of: Integer = Seq::index_of(
        Seq::concat(Seq::unit(x), Seq::unit(y)),
        Seq::unit(y),
        Integer::from(0),
    );
    let _index_of_default: Integer =
        Seq::index_of_default(Seq::concat(Seq::unit(x), Seq::unit(y)), Seq::unit(y));
